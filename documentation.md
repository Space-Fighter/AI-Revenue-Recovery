...
Last updated: 2026-09-05 — **Simulate tab: escalation is not a hard stop + voice-input removed** (plan.md §9):
`SimulateSession.tsx`'s `isActionable` was `outcome==='ongoing'||outcome==='ptp_promised'`, which locked the entire conversation (chat input, Next Turn, Auto-Run, every panel button) the instant a case escalated to human review — but escalation only means a ticket is open, not that the AI should go silent; the agent should keep answering in-scope queries (and can still help with a payment link if the customer changes their mind), same as a real support hand-off. Now `isActionable = outcome !== 'resolved' && outcome !== 'halted'` — `escalated` (any reason) stays fully interactive; only an actual payment (`resolved`) or a fraud/security halt (`halted`, per the project's existing "Recovery refuses to act" policy) locks the conversation for good. No backend change was needed — the S10 PTP state machine already lets a `ptp_promised`/`resolved` outcome win over the escalation ceiling, and `phase==='ended'` already kept the full 4-column UI rendered; the lock was purely the frontend flag. Also removed the now-fully-dead voice-input (Web Speech API) code from `SimulateSession.tsx` — the mic button lived only in the WhatsApp chat input bar, and voice playback was already gated to `channel==='call'` (permanently unreachable via Simulate since `begin()` forces `channel: 'message'`), so the whole `SpeechRecognition` apparatus (interfaces, `getSpeechRecognition`, `startListening`/`stopListening`, `isListening`/`speechSupported` state, `recognitionRef`) had no remaining UI entry point.
<!-- previous update: Simulate tab: bare-agreement override + bot-question fix -->

Previously (2026-09-05) — **Simulate tab: bare-agreement override + bot-question fix** (plan.md §9, `AGENTS_CONTRACT.md` §13 S11):
This deployment runs with `OPENROUTER_API_KEY` configured, so conversations are LLM-driven, not the deterministic fallback — and a live transcript showed the LLM's own judgment failing to recognize an unambiguous "okay" (in direct reply to the agent's own "should I send you the payment link?") as agreement, so the case never became a PTP and instead accumulated attempts until it hit the generic `max_attempts_exceeded` ceiling. Fixed with a deterministic backstop: `_is_clear_agreement`/`_apply_agreement_override` runs after every LLM call (success or fallback) in both `send_message` and `advance_conversation` — if the outcome is still `"ongoing"` and the customer's own message is an unambiguous agreement (same bare-word/`_AGREE_PHRASES` semantics as the existing fallback branch, factored out so both share one definition), the outcome is force-corrected to `"ptp_promised"` regardless of what the model decided, before the S10 PTP state machine runs. Never applied to the `speaker="agent"` human-takeover path (that outcome stays fully trusted as-supplied). Separately fixed: the deterministic fallback's question-detection previously treated *any* `"?"` as a request to re-explain the failure reason — so an off-topic "Are you a bot?" got the same recycled root-cause-explanation text instead of an honest answer; a new identity/bot-detection branch (`_IDENTITY_WORDS`) now answers on-topic, and the failure-explanation branch requires an actual `_QUESTION_WORDS` hit or a `"?"` paired with a `_FAILURE_DOMAIN_WORDS` hit. Also fixed the frontend's "Reminders Cap: X/3" badge (`SimulateSession.tsx`'s `renderRemindersMetric`), which was a disconnected heuristic (`Math.min(3, sim_day)`) that could show "3/3" even when the real backend reminder-cadence state (`sim_state.reminder_days`) hadn't actually exhausted anything — it now reads the real state, with a "paused (PTP)" note during a promise wait.
<!-- previous update: Simulate tab: PTP escalation bugfix + reminder cadence -->

Previously (2026-09-05) — **Simulate tab: PTP escalation bugfix + reminder cadence** (plan.md §9, `AGENTS_CONTRACT.md` §12/§13 S10):
...
| `src/components/SimulateSession.tsx` | **Rewritten 2026-09-05**: Liquid-glass 4-column layout (Transcripts / Agent Event Logs / phone mockup / Controls & Actions). Left columns: smartphone chat/call mockup with live voice/text interaction, takeover toggles, and barge-in support; structured transcripts and **Unified Audit & Event Trail** embedding `AuditTimeline` — pre-loads the case's historical DB decision trail and live-appends customer actions (link clicks, OTP entries, customer replies, silence) and agent decisions in strict chronological order. **Controls & Actions panel reworked (2026-09-05):** removed the redundant "Tester Input" box and the "Simulate Customer Actions" button group (the dead `logAction` no-op stub and its ~10 call sites also removed — `logSimulatedEvent` was always the real logging path); the "AI Simulation Engine" card is now one vertical stack in order Next Turn → Auto-Run → Out-of-Scope Query → Force Human Escalation → Record Promise to Pay (PTP). **PTP redesigned 2026-09-05 (S10, bugfix):** `ticketStatus` is now `'none'\|'ptp_pending'\|'human_review'\|'recovered'` — a Promise-to-Pay is its own "Awaiting Settlement" track, no longer conflated with human review (previously the banner said "Routed to Human Review" the instant a PTP was recorded, and a backend bug could immediately re-escalate it same-turn). `isActionable` gates Next Turn/Auto-Run/the chat input/PTP button — a PTP is a wait, not a stop, so the tester can keep advancing turns/days to watch it resolve (payment) or go overdue. `applyOutcome` logs `ptp_recorded` (not "ticket opened") when a promise starts, `ptp_broken` (not generic `escalation_triggered`) when a PTP specifically breaks, and `ticket_closed_recovered` on payment. `send`/`advance`/`playToResolution` now handle a `suppressed`/null-turn response (the backend's reminder-cadence guard) by skipping playback of a phantom duplicate bubble instead of crashing on a null turn, and `playToResolution`'s loop no longer stops on `ptp_promised` (only on a truly terminal outcome). Payment/mandate links inside any turn's text (`LINK_PATTERN`, matches `rzp.io`/`razorpay.me`) render as clickable buttons (`renderMessageText`) that open a new **embedded fake-checkout screen** swapped in for the WhatsApp body (`phoneView: 'chat'\|'checkout'`) — masked amount/customer header plus a tester-picked mistake list (`CHECKOUT_MISTAKE_OPTIONS`: succeed / wrong OTP / wrong email-password / press back / insufficient balance), each calling `handleCompletePayment(forcedReason)` → `dataSource.simulatePlaygroundPayment(..., forcedReason)`, with a "‹ Back to WhatsApp" bail-out that makes no API call. Failure reasons are labeled via `FAILURE_REASON_LABEL` in the audit trail (e.g. "Failed: Insufficient Balance"). **`begin()` now always requests `channel: 'message'`** from `startPlayground` (was: no channel passed, letting the backend's `pick_channel(event)` auto-select call vs. WhatsApp by root cause) — the embedded checkout and reworked panel only exist on the WhatsApp rail, so a call-mapped case (`insufficient_funds`, `card_declined`, etc.) would otherwise open on the voice-call screen with none of it available. **`renderRemindersMetric` fixed (2026-09-05, S11):** the "Reminders Cap: X/3" badge previously computed `remindersScheduled` from a disconnected heuristic (`Math.min(3, sim_day)`) that could show "3/3" regardless of what actually happened; it now reads the real backend cadence state (`simState.reminder_days.length`), with a "paused (PTP)" note appended while `outcome==='ptp_promised'` (no automated nudges are sent during that wait). **Escalation is not a hard stop (2026-09-05):** `isActionable` is now `outcome!=='resolved' && outcome!=='halted'` (was `outcome==='ongoing'||'ptp_promised'`) — a case escalated to human review stays fully interactive (chat, Next Turn, Auto-Run, every panel button) since the agent should keep answering in-scope queries and can still help with a payment link if the customer changes their mind; only an actual payment or a fraud/security halt locks the conversation. No backend change needed — the existing S10 state machine and `phase==='ended'` rendering already supported this, the lock was purely this one frontend flag. **Voice input removed (2026-09-05):** the mic button and the entire `SpeechRecognition` apparatus (`getSpeechRecognition`, `startListening`/`stopListening`, `isListening`/`speechSupported` state, `recognitionRef`, related type interfaces) were deleted — voice playback was already gated to `channel==='call'`, permanently unreachable since `begin()` forces `channel: 'message'`, so the mic (voice input) button in the WhatsApp chat input bar was its only remaining UI entry point. |
