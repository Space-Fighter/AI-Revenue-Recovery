...
| `src/components/SimulateSession.tsx` | **Rewritten 2026-09-05**: Liquid-glass two-column layout. Left column: smartphone chat/call mockup with live voice/text interaction, takeover toggles, and barge-in support. Right column: structured transcripts (Messaging & Call) and **Column 2: Unified Audit & Event Trail** embedding `AuditTimeline` — pre-loads the case's historical DB decision trail and live-appends customer actions (link clicks, OTP entries, customer replies, silence) and agent decisions in strict chronological order. |
...
| POST | `/api/events/{event_id}/playground/pay` | `routes.playground_pay` | body `{history, channel, sim_state?}` → `{turn, outcome, reasoning, history, sim_state, payment_id, amount, captured, reason, escalation?}` — calls `playground.click_payment_link` (pure `payment.resolve_fake_capture`, never `apply_capture`); weighted wrong_otp/insufficient_funds/user_cancelled/success | **done** |
