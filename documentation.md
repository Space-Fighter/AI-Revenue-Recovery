...
Last updated: 2026-09-05 — **Simulate tab Controls & Actions rework** (plan.md §9, Simulate/Playground UI):
Removed the redundant "Tester Input" box and "Simulate Customer Actions" button group from `SimulateSession.tsx` (and the dead `logAction` no-op stub + its ~10 call sites); the "AI Simulation Engine" card is now one vertical stack — Next Turn → Auto-Run → Out-of-Scope Query → Force Human Escalation → Record Promise to Pay (PTP). PTP now opens a simulated ticket routed to human review (`ticketStatus`), which clears to "recovered" once the customer later pays. Payment/mandate links in the transcript are now clickable and open a new embedded fake-checkout screen inside the phone mockup (`phoneView: 'chat'|'checkout'`) where the tester explicitly picks the outcome (succeed / wrong OTP / wrong email-password / press back / insufficient balance) — backed by a new `forced_reason` param on `playground.click_payment_link` (`backend/app/agents/playground.py`, `AGENTS_CONTRACT.md` §12/§13 S9). Insufficient-balance failures never escalate — they reschedule to a stated `salary_reminder_day` (sandbox stand-in for `recovery.SALARY_WINDOW_DAY`). `payment.py`/`payment_routes.py`/`PayCheckout.tsx` (the real DB-writing `/pay/:token` flow) untouched.
